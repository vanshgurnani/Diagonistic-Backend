...
module.exports.registerHandler = async (req, res) => {
  try {
      ...
      await dbUtils.updateOne({ email: data.email }, { otp: otp }, DATABASE_COLLECTIONS.USERS);

      const message = `${data.username || data.email} is successfully registered.`;

      // Send success response
      res.status(200).json({ type: "success" , message: message , newUser });
  } catch (error) {
      if (error.code === 11000 && error.keyPattern && error.keyPattern.username) {
          return res.status(400).json({ type: "Error", error: "Username is already taken." });
      }
...
module.exports.verifyOtpController = async (req, res) => {
  try {
    ...
    await dbUtils.updateOne({ email }, { otp: null }, DATABASE_COLLECTIONS.USERS);
...
module.exports.locationGet = async (req, res) => {
  try {
    ...
    const nominatimResponse = await axios.get(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}`);
...
module.exports.getProfileController = async (req, res) => {
  try {
    ...
    const userProfile = await dbUtils.findOne(userId, DATABASE_COLLECTIONS.USERS);

    // Check if user profile exists
    if (!userProfile) {
      return res.status(404).json({ type: 'Error', message: "User profile not found." });
    }
    ...
    res.status(500).json({ type: 'Error', message: "Failed to fetch user profile." });
  }
};
...
module.exports.updateProfileHandler = async (req, res) => {
  try {
      ...
      const updatedProfile = await dbUtils.updateOne(query, updatedProfileData, DATABASE_COLLECTIONS.USERS);

      // Check if profile was successfully updated
      if (updatedProfile) {
          res.status(200).json({ type: "Success", message: "Profile updated successfully.", data: updatedProfileData });
      } else {
          res.status(404).json({ type: "Error", error: "User profile not found or could not be updated." });
      }
  } catch (error) {
...
module.exports.findDistanceController = async (req, res) => {
  try {
      ...
      if (!lat1 || !lon1 || !lat2 || !lon2) {
          return res.status(400).json({ type: 'Error', message: 'Please provide all latitude and longitude coordinates.' });
      }
      ...
      const distance = await commonUtils.calculateDistance(latitude1, longitude1, latitude2, longitude2);
...
module.exports.loginHandler = async (req, res) => {
  try {
      ...
      if (!email || !password) {
          return res.status(400).json({ type: 'Error', message: "Email, password, and roles are required." });
      }
      ...
      if (!user) {
          return res.status(400).json({ type: 'Error', message: "Invalid email, password, or role." });
      }
